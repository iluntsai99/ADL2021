...
def main(args):
    ...
    id = []
    with torch.no_grad():
        for X_batch, Y_batch, text_lens, ids in tqdm(test_loader):
            X_batch = X_batch.to(device)
            outputs = model(X_batch)
            preds = outputs.argmax(dim=-1).int().tolist()
            truths = [truth[:text_len].tolist() for truth, text_len in zip(Y_batch, text_lens.tolist())]
            # truths = truths.cpu()
            # print(truths)
            # return
            # print(truth)
    ...
    truthss = [' '.join([dataset.idx2label(label_id) for label_id in truth]) for truth in truthss]
    prediction = [pred.split() for pred in prediction]
    truthss = [truth.split() for truth in truthss]
    print(classification_report(truthss, prediction, scheme=IOB2, mode='strict'))
    
    total, count = 0, 0
    for i in range(len(prediction)):
        for j in range(len(prediction[i])):
            total += 1
            count += 1 if prediction[i][j]==truthss[i][j] else 0
    print("token acc: {}".format(count / total))

    count = 0
    for i in range(len(prediction)):
        count += 1 if prediction[i]==truthss[i] else 0
    print("joint acc: {}".format(count / len(prediction)))
