...
def main(args):
    ...
    schedular = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.5, patience=10)
    ...
    for epoch in range(args.num_epoch):
        ...
        if optimizer.param_groups[0]["lr"] != lr_now:
            print("Learning rrate is changed to {} from {}".format(optimizer.param_groups[0]["lr"], lr_now))
            lr_now = optimizer.param_groups[0]["lr"]
        if lr_now < 1e-5 or early_stop > 15:
            print("Early stop!")
            break
