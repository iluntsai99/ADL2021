...
def main(args):
    ...
    model = slotClassifier(embeddings, hidden_size, num_layers, dropout, bidirectional, len(tag2idx), vocab.pad_id)
    ...
    for epoch in range(args.num_epoch):
        ...
        if (total_acc / dev_size) >= best_acc:
            ...
            torch.save(model.state_dict(), args.ckpt_dir / f"bestB32.pt")
            early_stop = 0
        else:
            early_stop += 1
        schedular.step(loss)
