...
def main(args):
    ...
    if (args.start_from_last):
        ...
        QA_Model = BertForQuestionAnswering.from_pretrained(args.ckpt_dir).to(device)
    else:
        QA_Model = BertForQuestionAnswering.from_pretrained("hfl/chinese-macbert-large").to(device)
    ...
    train_set = QA_Dataset(TRAIN, data[TRAIN], train_questions_tokenized, context_tokenized)
    dev_set = QA_Dataset(DEV, data[DEV], dev_questions_tokenized, context_tokenized)
    train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True)

    optimizer = AdamW(QA_Model.parameters(), lr=args.lr)
    print(args.lr)
    ...
    for epoch in range(args.num_epoch):
        ...
        for i, datas in enumerate(tqdm(train_loader)):
            ...
            output = QA_Model(input_ids=datas[0], token_type_ids=datas[1], attention_mask=datas[2], start_positions=datas[3], end_positions=datas[4])
            
            # Prediction is correct only if both start_index and end_index are correct
            start_index = torch.argmax(output.start_logits, dim=1)
            end_index = torch.argmax(output.end_logits, dim=1)
            train_acc += ((start_index == datas[3]) & (end_index == datas[4])).float().mean()
            ...
            if i % args.logging_step == 0 and i != 0:
                ...
                with torch.no_grad():
                    ...
                    for i, datas in enumerate(dev_loader):
                        datas = [data.to(device) for data in datas]
                        output = QA_Model(input_ids=datas[0].squeeze(dim=0), token_type_ids=datas[1].squeeze(dim=0), attention_mask=datas[2].squeeze(dim=0))
                        # prediction is correct only if answer text exactly matches
                        relevant = data[DEV][randomlist[i]]["relevant"]
                        dev_acc += (utils.evaluate(datas, output, context_tokenized[relevant], contexts[relevant]) == data[DEV][randomlist[i]]["answers"][0]["text"]) / len(dev_loader)
                    ...
                    if (dev_acc >= best_acc):
                        ...
                        best_acc = dev_acc
                        QA_Model.save_pretrained(args.ckpt_dir)
                # break
